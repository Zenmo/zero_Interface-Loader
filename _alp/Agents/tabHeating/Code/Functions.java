...
for (J_EAProfile j_ea : gc.c_profileAssets) {
	if (j_ea.getEnergyCarrier() == OL_EnergyCarriers.HEAT) {
		double maxValue = j_ea.getPeakPower_kW(); //j_ea.getProfileScaling_fr() * Arrays.stream(j_ea.a_energyProfile_kWh).max().getAsDouble();
...
for (GCHouse house: housesGCList ) {
	...
	// Else house has a customprofiel
		...
		J_EAProfile heatDemandProfile = (J_EAProfile)findFirst(house.c_profileAssets, x->x instanceof J_EAProfile && x.getEnergyCarrier() == OL_EnergyCarriers.HEAT);
