/**
 * J_GridOperatorTariffsStedin_2023
 */
...
public class J_GridOperatorTariffsStedin_2023 implements I_GridOperatorTariffs {
	//Data source: https://www.acm.nl/system/files/documents/tarievenblad-stedin-elektriciteit-2023.xlsx   (Bijlage 2a + 2b bij Tarievenbesluit Elektriciteit 2023 Stedin)
	//			&& https://www.stedin.net/tarieven/download-tarieven  (kleinverbruik tariefblad 2023, tabel Metertarieven)
	...
	double measurementServiceCostSmallConnection_eurpday = 13.77/365;	//meetdienst 13.77 EUR/jaar
	...
	Map<String, Double> periodicalPhyscialConnectionCapacityCostsTable_eurpyr = Map.ofEntries(
			Map.entry("t/m 1 x 6 A (geschakeld)", 12.6356 + 13.77),
			Map.entry("t/m 1 x 10 A", 34.05 + 13.77),
			Map.entry("> 1 x 10 A t/m 3 x 25 A", 34.05 + 13.77),
			Map.entry("> 3 x 25 A t/m 3 x 35 A", 55.85 + 13.77),
			Map.entry("> 3 x 35 A t/m 3 x 50 A", 55.85 + 13.77),
			Map.entry("> 3 x 50 A t/m 3 x 63 A", 55.85 + 13.77),
			Map.entry("> 3 x 63 A t/m 3 x 80 A", 55.85 + 13.77),
			Map.entry("> 3 x 80 A t/m 175 kVA", 121.8),
			Map.entry("> 175 kVA t/m 1.750 kVA", 1091.8),
			Map.entry("> 1.750 kVA t/m 5 MVA", 2650.0),
			Map.entry("> 5 MVA t/m 10 MVA", 13099.4412),
			Map.entry("> 10 MVA", 13099.4412)
			);

	Map<String, Double> transportCostsTable_smallConnections_eurpday = Map.ofEntries(
			Map.entry("t/m 1 x 6 A (geschakeld)", 2.84/365),
			Map.entry("t/m 1 x 10 A", 28.4/365),
			Map.entry("> 1 x 10 A t/m 3 x 25 A", 227.2/365),
			Map.entry("> 3 x 25 A t/m 3 x 35 A", 1136.0/365),
			Map.entry("> 3 x 35 A t/m 3 x 50 A", 1704.0/365),
			Map.entry("> 3 x 50 A t/m 3 x 63 A", 2272.0/365),
			Map.entry("> 3 x 63 A t/m 3 x 80 A", 2840.0/365)
			);

	Map<String, Double> transportCostsTable_largeConnections_normalTarif_eurpkWh = Map.ofEntries(
			Map.entry("LS (contract vermogen t/m 50 kW)", 0.0579),
			Map.entry("Trafo MS/LS (contract vermogen meer dan 50 kW t/m 150 kW)", 0.0148),
			Map.entry("MS (contract vermogen meer dan 150 kW t/m 1500 kW)", 0.0148),
	...
	Map<String, Double> transportCostsTable_largeConnections_lowTarif_eurpkWh = Map.ofEntries( //Between 23:00 and 7:00
			Map.entry("LS (contract vermogen t/m 50 kW)", 0.0357),
			Map.entry("Trafo MS/LS (contract vermogen meer dan 50 kW t/m 150 kW)", 0.0148),
			Map.entry("MS (contract vermogen meer dan 150 kW t/m 1500 kW)", 0.0148),
			Map.entry("Trafo HS+TS/MS > 1500 kW", 0.0),
			Map.entry("TS > 1500 kW", 0.0)
	...
	Map<String, Double> maxPeakPowerCostsTable_eurpkWpmonth = Map.ofEntries(
			...
			Map.entry("Trafo MS/LS (contract vermogen meer dan 50 kW t/m 150 kW)", 2.4264),
			Map.entry("MS (contract vermogen meer dan 150 kW t/m 1500 kW)", 2.4264),
			Map.entry("Trafo HS+TS/MS > 1500 kW", 4.0649),
			Map.entry("TS > 1500 kW", 4.0184)
			);
	Map<String, Double> contractCapacityCostsTable_eurpkWpyr = Map.ofEntries(
			Map.entry("LS (contract vermogen t/m 50 kW)", 14.2097),
			Map.entry("Trafo MS/LS (contract vermogen meer dan 50 kW t/m 150 kW)", 38.0567),
			Map.entry("MS (contract vermogen meer dan 150 kW t/m 1500 kW)", 19.3247),
			Map.entry("Trafo HS+TS/MS > 1500 kW", 36.5674),
			Map.entry("TS > 1500 kW", 35.3278)
			);
     ...
     * Default constructor
     */
    public J_GridOperatorTariffsStedin_2023() {
    }
